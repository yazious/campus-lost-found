...
//
// HOW TO RUN (this MUST run on a real Flutter engine — an emulator, a
// physical device, or Chrome — NOT plain `flutter test`, which runs on the
// bare Dart VM and has no platform channels. Firebase plugins talk to the
// native SDK through platform channels, so `flutter test` alone will always
// fail here with "Unable to establish connection on channel."):
//   1. Confirm firebase_options.dart points at your TEST project (not prod).
//   2. One-time setup: flutter pub add integration_test --sdk=flutter --dev
//   3. Create the folder `integration_test/` in your project root (a sibling
//      of `test/`, not inside it) and place this file there as:
//        integration_test/report_item_integration_test.dart
//   4. Start an emulator, or plug in a device, or just target Chrome.
//      Check available targets with: flutter devices
//   5. Run (pick the device id from step 4, or use "chrome"):
//        flutter test integration_test/report_item_integration_test.dart -d chrome
//   6. Send back the full console output.
